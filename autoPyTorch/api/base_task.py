...
from autoPyTorch.pipeline.components.setup.traditional_ml.traditional_learner import (
    get_available_traditional_learners,
)
...
def _pipeline_predict(
            ...
            if not ((prediction >= 0).all() and (prediction <= 1).all()):
                ...
                raise ValueError(
                    "For {}, prediction probability not within [0, 1]: {}/{}!".format(
                        pipeline, prediction, np.sum(prediction, axis=1)
                    )
                )
    if STRING_TO_TASK_TYPES.get(task_type, -1) != TIMESERIES_FORECASTING:
        if (
            ...
            or len(X_.shape) < 1
            or X_.shape[0] < 1
            or prediction.shape[0] != X_.shape[0]
        ):
            logger.warning(
...
class BaseTask(ABC):
    ...
    def __init__(
        ...
        if (
            isinstance(resampling_strategy, NoResamplingStrategyTypes)
            and ensemble_size != 0
        ):
            raise ValueError(
        ...
        self.pipeline_options = replace_string_bool_to_bool(
            json.load(
                open(
                    os.path.join(
        ...
        if search_space_updates is not None:
            if not isinstance(
                ...
                raise ValueError(
                    "Expected search space updates to be of instance"
    ...
    def build_pipeline(
        ...
        raise NotImplementedError(
            "Function called on BaseTask, this can only be called by "
    ...
    def set_pipeline_options(self, **pipeline_options_kwargs: Any) -> None:
        ...
        if len(unknown_keys) > 0:
            raise ValueError(
                "Invalid configuration arguments given {},"
                " expected arguments to be in {}".format(
                    unknown_keys, self.pipeline_options.keys()
                )
            )
    ...
    def get_search_space(self, dataset: BaseDataset = None) -> ConfigurationSpace:
        ...
        if self.search_space is not None:
            ...
            return get_configuration_space(
                info=dataset.get_dataset_properties(dataset_requirements),
                include=self.include_components,
                exclude=self.exclude_components,
                search_space_updates=self.search_space_updates,
            )
        raise ValueError(
            "No search space initialised and no dataset passed. "
    ...
    def _get_logger(self, name: str) -> PicklableClientLogger:
        ...
        # Without sleep(5) here the program may get stuck because of a race condition.
        # The logging server cannot acquire lock on the port.
        time.sleep(5)
    ...
    def _load_models(
        ...
        if self.resampling_strategy is None:
            raise ValueError(
                "Resampling strategy is needed to determine what models to load"
            )
    ...
    def _load_best_individual_model(self) -> SingleBest:
        ...
        if self._metric is None:
            raise ValueError(
                "Providing a metric to AutoPytorch is required to fit a model. "
    ...
    def _do_dummy_prediction(self) -> None:
        ...
        if status == StatusType.SUCCESS:
            ...
            if additional_info.get("exitcode") == -6:
                err_msg = (
                    "Dummy prediction failed with run state {},\n"
    ...
    def _do_traditional_prediction(
        self, time_left: int, func_eval_time_limit_secs: int
    ) -> None:
        """
        ...
        model_configs = [
            (classifier, self.pipeline_options[self.pipeline_options["budget_type"]])
        ...
        run_history.save_json(
            os.path.join(
                self._backend.internals_directory, "traditional_run_history.json"
            ),
            save_external=True,
        )
        return
    ...
    def _search(
        ...
        precision: int = 32,
        disable_file_output: Optional[
            List[Union[str, DisableFileOutputParameters]]
        ] = None,
        ...
        **kwargs: Any,
        ...
        if self.task_type != dataset.task_type:
            raise ValueError(
                "Incompatible dataset entered for current task,"
        ...
        self._disable_file_output = (
            disable_file_output if disable_file_output is not None else []
        )
        if (
            ...
            self._logger.warning(
                f"No ensemble will be created when {DisableFileOutputParameters.y_optimization}"
        ...
        self._metric = get_metrics(
            names=[optimize_metric], dataset_properties=dataset_properties
        )[0]

        self.pipeline_options["optimize_metric"] = optimize_metric

        if all_supported_metrics:
            self._scoring_functions = get_metrics(
                dataset_properties=dataset_properties, all_supported_metrics=True
            )
        else:
        ...
        if budget_type not in ("epochs", "runtime") and (
            ...
            raise ValueError(
                "Budget type must be one ('epochs', 'runtime')"
        ...
        if (
            func_eval_time_limit_secs is None
            or func_eval_time_limit_secs > time_left_for_modelfit
        ):
            self._logger.warning(
        ...
        if time_left_for_smac <= 0:
            ...
            try:
                (
                    run_history,
                    self._results_manager.trajectory,
                    budget_type,
                ...
                try:
                    ...
                    self._logger.warning(
                        f"Cannot save {trajectory_filename} due to {e}..."
                    )
            except Exception as e:
        ...
        if proc_ensemble is not None:
            self._results_manager.ensemble_performance_history = list(
            ...
            if len(proc_ensemble.futures) > 0:
                ...
                if result:
                    ...
                    self._results_manager.ensemble_performance_history.extend(
                        ensemble_history
                    )
    ...
    def _get_fit_dictionary(
        ...
        if dataset.test_tensors is not None:
            X_test = (
                dataset.test_tensors[0].copy()
                if dataset.test_tensors[0] is not None
                else None
            )
            y_test = (
                dataset.test_tensors[1].copy()
                if dataset.test_tensors[1] is not None
                else None
            )
        else:
            X_test = None
        ...
        X_train = (
            dataset.train_tensors[0].copy()
            if dataset.train_tensors[0] is not None
            else None
        )
        ...
        if STRING_TO_TASK_TYPES[self.task_type] == TIMESERIES_FORECASTING:
            warnings.warn(
                "Currently Time Series Forecasting tasks do not allow computing metrics "
    ...
    def refit(
        ...
        if dataset is None:
            if X_train is None and y_train is None:
                raise ValueError(
                    "No dataset provided, must provide X_train, y_train tensors"
                )
        ...
        pipeline_options = (
            self.pipeline_options.copy().update(pipeline_options)
        ...
        for _, run_value in run_history.data.items():
            ...
            if old_identifier_index is not None:
                ...
                warnings.warn(
                    f"Refit for {config} failed. Model fitted during search will be used instead."
                )
            old_identifier_index = None
        ...
        self.run_history.update(run_history, DataOrigin.EXTERNAL_SAME_INSTANCES)
        run_history.save_json(
            os.path.join(self._backend.internals_directory, "refit_run_history.json"),
            save_external=True,
        )
    ...
    def _get_fitted_pipeline(
        ...
        if run_value.status != StatusType.SUCCESS:
            warnings.warn(
                f"Fitting pipeline failed with status: {run_value.status}"
    ...
    def predict(
        ...
        if self.ensemble_ is None and not self._load_models():
            raise ValueError(
                "No ensemble found. Either fit has not yet "
        ...
        if isinstance(
            self.resampling_strategy, (HoldoutValTypes, NoResamplingStrategyTypes)
        ):
            models = self.models_
        ...
        all_predictions = joblib.Parallel(n_jobs=n_jobs)(
            joblib.delayed(_pipeline_predict)(
                ...
                batch_size,
                self._logger,
                STRING_TO_TASK_TYPES[self.task_type],
                self.task_type,
            )
        ...
        if len(all_predictions) == 0:
            raise ValueError(
                ...
                "%s" % (str(list(self.ensemble_.indices_)), str(list(self.models_)))
            )
    ...
    def score(
        self, y_pred: np.ndarray, y_test: Union[np.ndarray, pd.DataFrame]
    ) -> Dict[str, float]:
        ...
        if self._metric is None:
            raise ValueError(
                "No metric found. Either fit/search has not been called yet "
                "or AutoPyTorch failed to infer a metric from the dataset "
            )
        if self.task_type is None:
            raise ValueError(
                "AutoPytorch failed to infer a task type from the dataset "
    ...
    def get_incumbent_results(
        self, include_traditional: bool = False
        ...
        if self._metric is None:
            raise RuntimeError(
                "`search_results` is only available after a search has finished."
            )

        return self._results_manager.get_incumbent_results(
            metric=self._metric, include_traditional=include_traditional
        )
    ...
    def get_models_with_weights(self) -> List:
        ...
        models_with_weights: List[
            Tuple[float, BasePipeline]
        ] = self.ensemble_.get_models_with_weights(self.models_)
    ...
    def get_search_results(self) -> SearchResults:
        ...
        return self._results_manager.get_search_results(
            metric=self._metric, scoring_functions=self._scoring_functions
        )
    ...
    def plot_perf_over_time(
        ...
        **kwargs: Any,
        ...
        if len(self.ensemble_performance_history) == 0:
            raise RuntimeError(
                "Visualization is available only after ensembles are evaluated."
            )
